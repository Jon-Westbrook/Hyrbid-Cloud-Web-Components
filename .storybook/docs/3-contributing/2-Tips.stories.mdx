...
This codebase generates several applications:
  ...
  - The widget for the production/sandbos registy.
...
In order to ensure that all the applications can access the assets the developer will use an evironment variable `$PUBLIC_ASSETS_URL`. Each application has been configured to translate that into the appropriate URL.

You can find this configuration in `./storybook/main.js` in the `staticDirs` section. And in the `./scritps/ci/upload-public-dirs-to-cos.sh`.
