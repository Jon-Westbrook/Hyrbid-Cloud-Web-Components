...
<a href="./docs/my-widget.zip">This ZIP archive</a> contains all the code in
this guide at the time of this writing.
...
```console
./scripts/extract-all.sh my-widget
```
...
Spanish.decorators = [storyWithTranslation('es')];
...
<small style={{ fontSize: '0.8em' }}>* Note that content is not translated in
  this example for simplicity.</small>
...
```console
npx @js-widgets/webpack-cli --existing-registry="${PUBLIC_URL}/registry.json" --output-dir="${TRAVIS_BUILD_DIR}/widget-registry" "${TRAVIS_BUILD_DIR}"
```
