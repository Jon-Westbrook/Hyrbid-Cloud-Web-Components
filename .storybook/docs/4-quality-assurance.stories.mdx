...
Stories are renderings of React components with certain data and conditions. Storyshots will store the rendered markup in a file when the developer instructs it to do it. This is done by executing `yarn test --update` after making sure that Storybook behaves correctly in their local.
